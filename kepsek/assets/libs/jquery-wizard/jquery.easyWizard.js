	...
	var easyWizardMethods = {
		init : function(options) {
			var settings = $.extend( {
				...
				'showButtons' : true,
				'buttonsClass' : '',
				'prevButton' : '&laquo; Kembali',
				'nextButton' : 'Lanjut &raquo;',
				'debug' : false,
				'submitButton': false,
				...
				if(thisSettings.steps > 1) {
					...
					if(thisSettings.showButtons) {
						paginationHtml = '<div class="easyWizardButtons">';
							...
							//paginationHtml += thisSettings.submitButton?'<button type="submit" class="submit '+thisSettings.submitButtonClass+'"> <i class="fa fa-save"></i>'  +thisSettings.submitButtonText+'</button>':'';
						paginationHtml	+= '</div>';
